...
LanternDetectorNode::LanternDetectorNode(const rclcpp::NodeOptions& options)
    ...
    heartbeat_timer_ = this->create_wall_timer(std::chrono::duration<double>(heartbeat_period_s_),
                                               std::bind(&LanternDetectorNode::publish_heartbeat, this));

    auto qos = rclcpp::SensorDataQoS().get_rmw_qos_profile();

    sub_semantic_filter_ =
        std::make_shared<message_filters::Subscriber<sensor_msgs::msg::Image>>(this, semantic_topic, qos);
    sub_depth_filter_ =
        std::make_shared<message_filters::Subscriber<sensor_msgs::msg::Image>>(this, depth_topic, qos);

    sync_ = std::make_shared<message_filters::Synchronizer<SyncPolicy>>(SyncPolicy(10), *sub_semantic_filter_,
                                                                        *sub_depth_filter_);
    sync_->registerCallback(std::bind(&LanternDetectorNode::synchronized_callback, this,
    ...
    RCLCPP_INFO(this->get_logger(),
                "Lantern Detector Node initialized with "
    ...
    for (int y = 0; y < mask.rows; ++y) {
        for (int x = 0; x < mask.cols; ++x) {
            if (mask.at<uchar>(y, x) > 0) {
                ...
                if (std::isfinite(depth_value_m) && depth_value_m > min_depth_m_ &&
                    depth_value_m < max_depth_m_) {
    ...
    if (valid_pixel_count > 100) {
        ...
        try {
            const auto point_world =
                tf_buffer_->transform(point_depth_frame, world_frame_, tf2::durationFromSec(0.2));
...
void LanternDetectorNode::update_lanterns(const geometry_msgs::msg::Point& new_position) {
    auto iter = std::find_if(detected_lanterns_.begin(), detected_lanterns_.end(), [&](const auto& l) {
        double distance_m = std::hypot(l.position.x - new_position.x, l.position.y - new_position.y,
