#include "statemachine_pkg/backup_statemachine.h"
...
StateMachine::StateMachine() : rclcpp::Node("state_machine_node") {
    // Topics the other packages should subscribe/publish to are all prefixed with
    // "statemachine/" for commands and status, and package-specific "/ready" or
    // "/done" flags.
    ...
    topic_cmd_basic_waypoint_ =
        declare_parameter<std::string>("topic_cmd_basic_waypoint", "statemachine/cmd/basic_waypoint");
    topic_cmd_path_planning_ =
        declare_parameter<std::string>("topic_cmd_path_planning", "statemachine/cmd/path_planning");
    topic_cmd_mapping_ = declare_parameter<std::string>("topic_cmd_mapping", "statemachine/cmd/mapping");
    topic_cmd_controller_ =
        declare_parameter<std::string>("topic_cmd_controller", "statemachine/cmd/controller");
    topic_cmd_lantern_detector_ =
        declare_parameter<std::string>("topic_cmd_lantern_detector", "statemachine/cmd/lantern_detector");
    ...
    topic_lantern_detections_ =
        declare_parameter<std::string>("topic_lantern_detections", "detected_lanterns");
    ...
    sub_start_ = create_subscription<std_msgs::msg::Bool>(
        topic_start_, 10, [this](const std_msgs::msg::Bool::SharedPtr msg) {
    ...
    sub_abort_ = create_subscription<std_msgs::msg::Bool>(
        topic_abort_, 10, [this](const std_msgs::msg::Bool::SharedPtr msg) {
    ...
    sub_mapping_ready_ = create_subscription<std_msgs::msg::Bool>(
        topic_mapping_ready_, 10, [this](const std_msgs::msg::Bool::SharedPtr msg) {
    ...
    sub_path_ready_ = create_subscription<std_msgs::msg::Bool>(
        topic_path_ready_, 10, [this](const std_msgs::msg::Bool::SharedPtr msg) {
    ...
    sub_waypoint_done_ = create_subscription<std_msgs::msg::Bool>(
        topic_waypoint_done_, 10, [this](const std_msgs::msg::Bool::SharedPtr msg) {
    ...
    sub_goal_reached_ = create_subscription<std_msgs::msg::Bool>(
        topic_goal_reached_, 10, [this](const std_msgs::msg::Bool::SharedPtr msg) {
...
void StateMachine::onTimer() {
    ...
    if (mission_start_time_.nanoseconds() > 0 && mission_time_limit_sec_ > 0.0) {
        ...
        if (mission_elapsed >= mission_time_limit_sec_ && state_ != MissionState::RETURN_HOME &&
            state_ != MissionState::LAND && state_ != MissionState::DONE) {
...
void StateMachine::logLanternPose(const geometry_msgs::msg::PoseStamped& pose, int id, size_t count) {
    ...
    std::vector<std::string> lines;
    {
        ...
        if (in.good()) {
            while (std::getline(in, line)) {
                ...
                if (line.rfind("stamp_sec,", 0) == 0 || line.rfind("timestamp,", 0) == 0 ||
    ...
    const std::string new_line = id_str + "," + date_str + "," + username_ + "," + std::to_string(count) +
...
void StateMachine::checkSignalTimeout(const std::string& name, const rclcpp::Time& last_time,
                                      bool& reported) {
    if (reported) {
        return;
    }
...
void StateMachine::publishState() {
    if (lantern_pending_) {
        ...
        if (is_new) {
            ...
            RCLCPP_INFO(get_logger(), "Lantern detected at [%.2f, %.2f, %.2f] -> %s",
                        pending_lantern_.pose.position.x, pending_lantern_.pose.position.y,
                        pending_lantern_.pose.position.z, lantern_log_path_.c_str());
...
void StateMachine::onLanternDetections(const geometry_msgs::msg::PoseArray::SharedPtr msg) {
    ...
    logEvent(topic_lantern_detections_ +
             " -> received pose array (size=" + std::to_string(msg->poses.size()) + ")");
    ...
    for (const auto& pose : msg->poses) {
        ...
        logEvent(topic_lantern_detections_ + " -> received pose array of lantern " + std::to_string(id) +
                 " at [" + std::to_string(pose.position.x) + ", " + std::to_string(pose.position.y) + ", " +
...
int StateMachine::associateLantern(const geometry_msgs::msg::Point& pos, bool& is_new,
