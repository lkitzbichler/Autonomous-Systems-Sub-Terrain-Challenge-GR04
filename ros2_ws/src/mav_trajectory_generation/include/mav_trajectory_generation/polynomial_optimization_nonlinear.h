...
class PolynomialOptimizationNonLinear {
   ...
   public:
    ...
    void getTrajectory(Trajectory* trajectory) const { poly_opt_.getTrajectory(trajectory); }

    // Returns a const reference to the underlying linear optimization
    // object.
    const PolynomialOptimization<N>& getPolynomialOptimizationRef() const { return poly_opt_; }

    // Returns a non-const reference to the underlying linear optimization
    // object.
    PolynomialOptimization<N>& getPolynomialOptimizationRef() { return poly_opt_; }
   ...
   private:
    ...
    // Input: Custom data pointer = In our case, it's an ConstraintData object.
    // Output: Cost = based on the parameters passed in.
    static double objectiveFunctionTimeMellingerOuterLoop(const std::vector<double>& segment_times,
                                                          std::vector<double>& gradient, void* data);
    ...
    // object.
    // Output: Cost based on the parameters passed in.
    static double objectiveFunctionTimeAndConstraints(const std::vector<double>& optimization_variables,
                                                      std::vector<double>& gradient, void* data);
    ...
    static double evaluateMaximumMagnitudeConstraint(const std::vector<double>& optimization_variables,
                                                     std::vector<double>& gradient, void* data);
