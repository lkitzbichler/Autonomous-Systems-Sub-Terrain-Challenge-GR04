...
template <int _N>
bool PolynomialOptimization<_N>::setupFromVertices(const Vertex::Vector& vertices,
        ...
        for (Vertex::Constraints::const_iterator it = vertex.cBegin(); it != vertex.cEnd(); ++it) {
            if (it->first > kHighestDerivativeToOptimize) {
                ...
                LOG(WARNING) << "Invalid constraint on vertex " << vertex_idx
                             << ": maximum possible derivative is " << kHighestDerivativeToOptimize
...
template <int _N>
void PolynomialOptimization<_N>::invertMappingMatrix(const SquareMatrix& mapping_matrix,
...
void PolynomialOptimization<_N>::setupConstraintReorderingMatrix() {
    ...
    constraint_reordering_ =
        Eigen::SparseMatrix<double>(n_all_constraints_, n_fixed_constraints_ + n_free_constraints_);

    for (Eigen::VectorXd& df : fixed_constraints_compact_) df.resize(n_fixed_constraints_, Eigen::NoChange);
...
void PolynomialOptimization<_N>::updateSegmentTimes(const std::vector<double>& segment_times) {
    ...
    CHECK(n_segment_times == n_segments_) << "Number of segment times (" << n_segment_times
...
bool PolynomialOptimization<_N>::solveLinear() {
    ...
    Eigen::SparseMatrix<double> Rpp =
        R.block(n_fixed_constraints_, n_fixed_constraints_, n_free_constraints_, n_free_constraints_);
...
bool PolynomialOptimization<_N>::computeSegmentMaximumMagnitudeCandidates(const Segment& segment,
...
template <int _N>
bool PolynomialOptimization<_N>::computeSegmentMaximumMagnitudeCandidates(int derivative,
...
template <int _N>
template <int Derivative>
void PolynomialOptimization<_N>::computeSegmentMaximumMagnitudeCandidatesBySampling(
    const Segment& segment, double t_start, double t_stop, double dt, std::vector<double>* candidates) {
...
template <int _N>
Extremum PolynomialOptimization<_N>::computeMaximumOfMagnitude(int derivative,
    ...
    const Extremum candidate(segments_.back().getTime(),
...
template <int _N>
void PolynomialOptimization<_N>::computeQuadraticCostJacobian(int derivative, double t,
    ...
    for (int col = 0; col < N - derivative; col++) {
        for (int row = 0; row < N - derivative; row++) {
            ...
            (*cost_jacobian)(N - 1 - row, N - 1 - col) =
                ...
                Polynomial::base_coefficients_(derivative, N - 1 - col) * pow(t, exponent) * 2.0 / exponent;
        }
    }
}
