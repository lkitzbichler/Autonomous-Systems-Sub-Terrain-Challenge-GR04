...
inline std::ostream& operator<<(std::ostream& stream, const OptimizationInfo& val) {
    ...
    for (const std::pair<int, Extremum>& m : val.maxima) {
        stream << "    " << positionDerivativeToString(m.first) << ": " << m.second.value << " in segment "
               << m.second.segment_idx << " and segment time " << m.second.time << std::endl;
    }
    return stream;
}
...
template <int _N>
bool PolynomialOptimizationNonLinear<_N>::setupFromVertices(const Vertex::Vector& vertices,
    ...
    switch (optimization_parameters_.time_alloc_method) {
        ...
        default:
            n_optimization_parameters =
                segment_times.size() + poly_opt_.getNumberFreeConstraints() * poly_opt_.getDimension();
            break;
    }
...
int PolynomialOptimizationNonLinear<_N>::optimize() {
    ...
    optimization_info_.optimization_time =
        std::chrono::duration_cast<std::chrono::duration<double>>(t_stop - t_start).count();
...
int PolynomialOptimizationNonLinear<_N>::optimizeTimeMellingerOuterLoop() {
    ...
    try {
        ...
        nlopt_->set_min_objective(
            &PolynomialOptimizationNonLinear<N>::objectiveFunctionTimeMellingerOuterLoop, this);
    } catch (std::exception& e) {
        LOG(ERROR) << "error while setting up nlopt: " << e.what() << std::endl;
        return nlopt::FAILURE;
    }
    ...
    try {
        ...
        LOG(ERROR) << "error while running nlopt: " << e.what()
                   << ". This likely means the optimization aborted early." << std::endl;
    ...
    if (optimization_parameters_.print_debug_info_time_allocation) {
        ...
        std::for_each(original_segment_times.cbegin(), original_segment_times.cend(),
                      [](double c) { std::cout << c << " "; });
        std::cout << std::endl;
        std::cout << "[MEL RELATIVE Solution]: ";
        std::for_each(relative_segment_times.cbegin(), relative_segment_times.cend(),
                      [](double c) { std::cout << c << " "; });
        std::cout << std::endl;
        ...
        std::cout << "[MEL   Trajectory Time] Before: "
                  << std::accumulate(original_segment_times.begin(), original_segment_times.end(), 0.0)
                  << " | After Rel Change: "
                  << std::accumulate(relative_segment_times.begin(), relative_segment_times.end(), 0.0)
                  << " | After Scaling: "
                  << std::accumulate(scaled_segment_times.begin(), scaled_segment_times.end(), 0.0)
                  << std::endl;
    }
...
double PolynomialOptimizationNonLinear<_N>::getTotalCostWithSoftConstraints() const {
    ...
    double cost_constraints = evaluateMaximumMagnitudeAsSoftConstraint(
        inequality_constraints_, optimization_parameters_.soft_constraint_weight, 1e9);
...
void PolynomialOptimizationNonLinear<_N>::scaleSegmentTimesWithViolation() {
    ...
    if (optimization_parameters_.print_debug_info_time_allocation) {
        ...
        std::cout << "[Time Scaling] Beginning:  v: max: " << v_max_actual << " / " << v_max
                  << " a: max: " << a_max_actual << " / " << a_max << std::endl;
    }
    ...
    if (optimization_parameters_.print_debug_info_time_allocation) {
        double v_max_actual, a_max_actual;
        traj.computeMaxVelocityAndAcceleration(&v_max_actual, &a_max_actual);
        std::cout << "[Time Scaling] End: v: max: " << v_max_actual << " / " << v_max
                  << " a: max: " << a_max_actual << " / " << a_max << std::endl;
    }
}
...
int PolynomialOptimizationNonLinear<_N>::optimizeTimeAndFreeConstraints() {
    ...
    try {
        ...
        nlopt_->set_min_objective(&PolynomialOptimizationNonLinear<N>::objectiveFunctionTimeAndConstraints,
                                  this);
    } catch (std::exception& e) {
        LOG(ERROR) << "error while setting up nlopt: " << e.what() << std::endl;
        return nlopt::FAILURE;
...
template <int _N>
bool PolynomialOptimizationNonLinear<_N>::addMaximumMagnitudeConstraint(int derivative,
    ...
    if (!optimization_parameters_.use_soft_constraints) {
        try {
            nlopt_->add_inequality_constraint(
                &PolynomialOptimizationNonLinear<N>::evaluateMaximumMagnitudeConstraint,
                constraint_data.get(), optimization_parameters_.inequality_constraint_tolerance);
        } catch (std::exception& e) {
...
template <int _N>
double PolynomialOptimizationNonLinear<_N>::objectiveFunctionTime(const std::vector<double>& segment_times,
    ...
    if (optimization_data->optimization_parameters_.print_debug_info) {
        std::cout << "---- cost at iteration " << optimization_data->optimization_info_.n_iterations
                  << "---- " << std::endl;
    ...
    if (optimization_data->optimization_parameters_.use_soft_constraints) {
        cost_constraints = optimization_data->evaluateMaximumMagnitudeAsSoftConstraint(
...
template <int _N>
double PolynomialOptimizationNonLinear<_N>::objectiveFunctionTimeMellingerOuterLoop(
    const std::vector<double>& segment_times, std::vector<double>& gradient, void* data) {
...
template <int _N>
double PolynomialOptimizationNonLinear<_N>::objectiveFunctionTimeAndConstraints(const std::vector<double>& x,
...
double PolynomialOptimizationNonLinear<_N>::evaluateMaximumMagnitudeAsSoftConstraint(
    const std::vector<std::shared_ptr<ConstraintData>>& inequality_constraints, double weight,
    double maximum_cost) const {
    std::vector<double> dummy;
    double cost = 0;

    if (optimization_parameters_.print_debug_info) std::cout << "  soft_constraints: " << std::endl;
    ...
    for (std::shared_ptr<const ConstraintData> constraint : inequality_constraints_) {
        ...
        if (optimization_parameters_.print_debug_info) {
            std::cout << "    derivative " << constraint->derivative << " abs violation: " << abs_violation
                      << " : relative violation: " << relative_violation << " cost: " << current_cost
                      << std::endl;
        }
    }
    return cost;
...
template <int _N>
void PolynomialOptimizationNonLinear<_N>::setFreeEndpointDerivativeHardConstraints(
    const Vertex::Vector& vertices, std::vector<double>* lower_bounds, std::vector<double>* upper_bounds) {
