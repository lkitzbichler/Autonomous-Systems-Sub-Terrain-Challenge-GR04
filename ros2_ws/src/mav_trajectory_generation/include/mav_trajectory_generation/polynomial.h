...
class Polynomial {
   public:
    ...
    inline bool operator==(const Polynomial& rhs) const { return coefficients_ == rhs.coefficients_; }
        ...
        if (derivative == 0) {
            ...
            result.head(N_ - derivative) =
                coefficients_.tail(N_ - derivative)
                    .cwiseProduct(
                        base_coefficients_.block(derivative, derivative, 1, N_ - derivative).transpose());
            return result;
        }
    }
    ...
    bool selectMinMaxFromRoots(double t_start, double t_end, int derivative,
                               const Eigen::VectorXcd& roots_derivative_of_derivative,
    ...
    // Returns the minimum and maximum as pair<t, value>.
    bool computeMinMax(double t_start, double t_end, int derivative, std::pair<double, double>* minimum,
