...
class TrajectorySamplerNode : public rclcpp::Node {
   ...
   private:
    // Callbacks
    void pathSegmentsCallback(const mav_planning_msgs::msg::PolynomialTrajectory::SharedPtr segments_message);

    void pathSegments4DCallback(
        const mav_planning_msgs::msg::PolynomialTrajectory4D::SharedPtr segments_message);

    void stopSamplingCallback(const std::shared_ptr<rmw_request_id_t> request_header,
