...
bool sampleTrajectoryInRange(const Trajectory& trajectory, double min_time, double max_time,
    ...
    if (min_time < trajectory.getMinTime() || max_time > trajectory.getMaxTime()) {
        LOG(ERROR) << "Sample time should be within [" << trajectory.getMinTime() << " "
                   << trajectory.getMaxTime() << "] but is [" << min_time << " " << max_time << "]";
        return false;
    }
    ...
    std::vector<Eigen::VectorXd> position, velocity, acceleration, jerk, snap, yaw, yaw_rate;

    trajectory.evaluateRange(min_time, max_time, sampling_interval, derivative_order::POSITION, &position);
    trajectory.evaluateRange(min_time, max_time, sampling_interval, derivative_order::VELOCITY, &velocity);
    trajectory.evaluateRange(min_time, max_time, sampling_interval, derivative_order::ACCELERATION,
                             &acceleration);
    trajectory.evaluateRange(min_time, max_time, sampling_interval, derivative_order::JERK, &jerk);
    trajectory.evaluateRange(min_time, max_time, sampling_interval, derivative_order::SNAP, &snap);
    ...
    for (size_t i = 0; i < n_samples; ++i) {
        ...
        state.time_from_start_ns =
            static_cast<int64_t>((min_time + sampling_interval * i) * kNumNanosecondsPerSecond);
        if (trajectory.D() == 4) {
            ...
            state.angular_acceleration_W =
                mav_msgs::omegaDotFromRotationVector(rot_vec, rot_vec_vel, rot_vec_acc);
...
bool sampleWholeTrajectory(const Trajectory& trajectory, double sampling_interval,
    ...
    return sampleTrajectoryInRange(trajectory, min_time, max_time, sampling_interval, states);
}
...
bool sampleSegmentAtTime(const Segment& segment, double sample_time, mav_msgs::EigenTrajectoryPoint* state) {
    ...
    if (sample_time < 0.0 || sample_time > segment.getTime()) {
        LOG(ERROR) << "Sample time should be within [" << 0.0 << " " << segment.getTime() << "] but is "
                   << sample_time;
        return false;
    }
