...
TrajectorySamplerNode::TrajectorySamplerNode(const rclcpp::NodeOptions& options)
    ...
    stop_srv_ = this->create_service<std_srvs::srv::Empty>(
        "stop_sampling", std::bind(&TrajectorySamplerNode::stopSamplingCallback, this, std::placeholders::_1,
                                   std::placeholders::_2, std::placeholders::_3));

    // Client to take over publishing commands
    position_hold_client_ = this->create_client<std_srvs::srv::Empty>("back_to_position_hold");

    // Timer: create but don't start immediately (cancel, then reset when needed)
    publish_timer_ = this->create_wall_timer(std::chrono::duration<double>(dt_),
                                             std::bind(&TrajectorySamplerNode::commandTimerCallback, this));
    publish_timer_->cancel();
    heartbeat_timer_ =
        this->create_wall_timer(std::chrono::duration<double>(heartbeat_period_sec_),
...
void TrajectorySamplerNode::pathSegmentsCallback(
    ...
    if (segments_message->segments.empty()) {
        ...
        RCLCPP_INFO(get_logger(), "Trajectory sampler: received %zu waypoints",
...
void TrajectorySamplerNode::pathSegments4DCallback(
    ...
    if (segments_message->segments.empty()) {
        ...
        RCLCPP_INFO(get_logger(), "Trajectory sampler (4D): received %zu waypoints",
                    segments_message->segments.size());
    }
...
void TrajectorySamplerNode::processTrajectory() {
    ...
    if (position_hold_client_) {
        if (position_hold_client_->wait_for_service(0s)) {
            ...
            RCLCPP_WARN(get_logger(),
                        "Service 'back_to_position_hold' not available "
                        "when starting trajectory sampling.");
        }
    }
    ...
    if (publish_whole_trajectory_) {
        ...
        mav_trajectory_generation::sampleWholeTrajectory(trajectory_, dt_, &trajectory_points);
...
void TrajectorySamplerNode::commandTimerCallback() {
    if (current_sample_time_ <= trajectory_.getMaxTime()) {
        ...
        bool success = mav_trajectory_generation::sampleTrajectoryAtTime(trajectory_, current_sample_time_,
                                                                         &trajectory_point);
        if (!success) {
            RCLCPP_WARN(get_logger(), "Failed to sample trajectory at time %f, stopping timer.",
