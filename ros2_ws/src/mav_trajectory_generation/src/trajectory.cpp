...
void Trajectory::evaluateRange(double t_start, double t_end, double dt, int derivative_order,
...
bool Trajectory::getTrajectoryWithAppendedDimension(const Trajectory& trajectory_to_append,
                                                    Trajectory* new_trajectory) const {
...
bool Trajectory::computeMinMaxMagnitude(int derivative, const std::vector<int>& dimensions, Extremum* minimum,
    ...
    for (size_t segment_idx = 0; segment_idx < segments_.size(); segment_idx++) {
        ...
        if (!segments_[segment_idx].computeMinMaxMagnitudeCandidates(
                derivative, 0.0, segments_[segment_idx].getTime(), dimensions, &candidates)) {
            return false;
        }
        // Evaluate candidates.
        Extremum minimum_candidate, maximum_candidate;
        if (!segments_[segment_idx].selectMinMaxMagnitudeFromCandidates(
                derivative, 0.0, segments_[segment_idx].getTime(), dimensions, candidates, &minimum_candidate,
                &maximum_candidate)) {
            return false;
        }
...
bool Trajectory::getVertices(int max_derivative_order_pos, int max_derivative_order_yaw,
                             Vertex::Vector* pos_vertices, Vertex::Vector* yaw_vertices) const {
...
bool Trajectory::computeMaxVelocityAndAcceleration(double* v_max, double* a_max) const {
    ...
    bool success = computeMinMaxMagnitude(mav_trajectory_generation::derivative_order::VELOCITY, dimensions,
                                          &v_min_traj, &v_max_traj);
    success &= computeMinMaxMagnitude(mav_trajectory_generation::derivative_order::ACCELERATION, dimensions,
