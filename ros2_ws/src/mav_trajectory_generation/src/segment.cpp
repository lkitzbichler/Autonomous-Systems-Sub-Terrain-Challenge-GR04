...
bool Segment::computeMinMaxMagnitudeCandidateTimes(int derivative, double t_start, double t_end,
    ...
    if (dimensions.empty()) {
        ...
        for (int dim : dimensions) {
            if (dim < 0 || dim >= D_) {
                LOG(WARNING) << "Specified dimensions " << dim << " are out of bounds [0.." << D_ - 1 << "]."
                             << std::endl;
                return false;
            }
        ...
        // derivative.
        if (!polynomials_[dimensions[0]].computeMinMaxCandidates(t_start, t_end, derivative,
                                                                 candidate_times)) {
            return false;
        }
    }
...
bool Segment::computeMinMaxMagnitudeCandidates(int derivative, double t_start, double t_end,
                                               const std::vector<int>& dimensions,
...
bool Segment::selectMinMaxMagnitudeFromCandidates(int derivative, double t_start, double t_end,
                                                  const std::vector<int>& dimensions,
    ...
    if (N_ == segment_to_append.N()) {
        ...
        for (int i = 0; i < new_D; i++) {
            ...
            if (i < D_) {
                ...
                if (!segment_to_append[i - D_].getPolynomialWithAppendedCoefficients(new_N,
                                                                                     &polynomial_to_append)) {
                    return false;
                }
            }
