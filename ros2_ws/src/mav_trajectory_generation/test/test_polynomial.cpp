...
void findMinMaxBySampling(const Polynomial& polynomial, int derivative, double t_start, double t_end,
...
TEST(PolynomialTest, Convolution) {
    ...
    expected_convolution << coeffs_1(0) * coeffs_2(0), coeffs_1(0) * coeffs_2(1) + coeffs_1(1) * coeffs_2(0),
...
TEST(PolynomialTest, FindMinMax) {
    ...
    std::vector<int> derivatives_to_test = {derivative_order::POSITION, derivative_order::VELOCITY,
    ...
    for (int derivative : derivatives_to_test) {
        for (size_t i = 0; i < kNumPolynomials; i++) {
            ...
            std::pair<double, double> min_sampling, max_sampling, min_computing, max_computing;
            ...
            findMinMaxBySampling(p, derivative, t_start, t_end, &min_sampling, &max_sampling);
            timer_sampling.Stop();
            timing::Timer timer_analytic("find_min_max_analytic");
            bool success = p.computeMinMax(t_start, t_end, derivative, &min_computing, &max_computing);
            timer_analytic.Stop();
            if (!success) {
                std::cout << "Failed to compute roots of derivative of polynomial: " << coeffs.transpose()
                          << std::endl;
                num_failures++;
                continue;
            }
    ...
    std::cout << "Failed to compute minimum for " << num_failures << " / " << kNumPolynomials
