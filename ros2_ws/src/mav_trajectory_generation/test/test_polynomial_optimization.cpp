...
std::ostream& operator<<(std::ostream& stream, const OptimizationParams& val) {
    stream << "D: " << val.D << " max_deriv: " << val.max_derivative << " num_seg: " << val.num_segments
           << " seed: " << val.seed << " pos_bounds: " << val.pos_bounds << " v_max: " << val.v_max
           << " a_max: " << val.a_max;
    return stream;
}

// Parameters to test: N, max_derivative, num_segments, seed
class PolynomialOptimizationTests : public ::testing::TestWithParam<OptimizationParams> {
   public:
    void SetUp() override {
        params_ = GetParam();
        ...
        vertices_ = createRandomVertices(getHighestDerivativeFromN(N), params_.num_segments, pos_min, pos_max,
                                         params_.seed);
    }

    // Helper checking functions.
    void checkPath(const Vertex::Vector& vertices, const std::vector<Segment>& segments) const;
    bool checkCost(double cost_to_check, const Trajectory& trajectory, size_t derivative,
                   double relative_tolerance) const;
    void getMaxVelocityAndAccelerationAnalytical(const Trajectory& trajectory, double* v_max,
                                                 double* a_max) const;
    void getMaxVelocityAndAccelerationNumerical(const Trajectory& trajectory, double* v_max,
                                                double* a_max) const;
    bool checkExtrema(const std::vector<double>& testee, const std::vector<double>& reference,
...
void PolynomialOptimizationTests::checkPath(const Vertex::Vector& vertices,
    ...
    for (size_t i = 0; i < n_segments; ++i) {
        ...
        for (Vertex::Constraints::const_iterator it = v_begin.cBegin(); it != v_begin.cEnd(); ++it) {
        ...
        for (Vertex::Constraints::const_iterator it = v_end.cBegin(); it != v_end.cEnd(); ++it) {
            const int derivative = it->first;
            const Vertex::ConstraintValue desired = it->second;
            const Vertex::ConstraintValue actual = segment.evaluate(segment.getTime(), derivative);
            std::stringstream segment_derivative;
            printSegment(segment_derivative, segment, derivative);
            EXPECT_TRUE(EIGEN_MATRIX_NEAR(desired, actual, tol))
        ...
        if (i > 0) {
            ...
            for (size_t derivative = 0; derivative < N / 2; ++derivative) {
                ...
                const Vertex::ConstraintValue current_segment_value = segment.evaluate(0, derivative);
                std::stringstream segment_derivative;
                printSegment(segment_derivative, segment, derivative);
                EXPECT_TRUE(EIGEN_MATRIX_NEAR(last_segment_value, current_segment_value, tol))
...
bool PolynomialOptimizationTests::checkCost(double cost_to_check, const Trajectory& trajectory,
    ...
    double cost_numeric = computeCostNumeric(trajectory, derivative, sampling_interval);

    if (std::abs(cost_numeric - cost_to_check) > cost_numeric * relative_tolerance) {
        std::cout << "[FAIL]: tested cost is" << cost_to_check << " but real cost is " << cost_numeric
                  << ". Difference vs. allowed difference: " << std::abs(cost_numeric - cost_to_check) << "/"
...
void PolynomialOptimizationTests::getMaxVelocityAndAccelerationAnalytical(const Trajectory& trajectory,
                                                                          double* v_max,
                                                                          double* a_max) const {
    ...
    trajectory.computeMinMaxMagnitude(mav_trajectory_generation::derivative_order::VELOCITY, dimensions,
                                      &v_min_traj, &v_max_traj);
    trajectory.computeMinMaxMagnitude(mav_trajectory_generation::derivative_order::ACCELERATION, dimensions,
...
void PolynomialOptimizationTests::getMaxVelocityAndAccelerationNumerical(const Trajectory& trajectory,
                                                                         double* v_max, double* a_max) const {
    *v_max = getMaximumMagnitude(trajectory, mav_trajectory_generation::derivative_order::VELOCITY);
    *a_max = getMaximumMagnitude(trajectory, mav_trajectory_generation::derivative_order::ACCELERATION);
}

bool PolynomialOptimizationTests::checkExtrema(const std::vector<double>& testee,
                                               const std::vector<double>& reference, double tol) const {
    for (double t : testee) {
        ...
        if (!found_match) {
            std::cout << "[ERROR]: did not find matching extremum for " << t << " in ";
            for (double r : reference) {
                std::cout << r << " ";
            }
            std::cout << std::endl;
...
TEST_P(PolynomialOptimizationTests, VertexGeneration) {
    ...
    EXPECT_EQ(vertices_.front().getNumberOfConstraints(), getHighestDerivativeFromN(N) + 1);
    EXPECT_EQ(vertices_.back().getNumberOfConstraints(), getHighestDerivativeFromN(N) + 1);
...
TEST_P(PolynomialOptimizationTests, UnconstrainedLinearEstimateSegmentTimes) {
    std::vector<double> segment_times = estimateSegmentTimes(vertices_, v_max, a_max);
    ...
    std::cout << "Base coefficients: " << Polynomial::base_coefficients_.block(3, 0, 1, N) << std::endl;

    checkPath(vertices_, segments);
    double v_max_traj = getMaximumMagnitude(trajectory, derivative_order::VELOCITY);
    double a_max_traj = getMaximumMagnitude(trajectory, derivative_order::ACCELERATION);
...
TEST_P(PolynomialOptimizationTests, ExtremaOfMagnitude) {
    ...
    for (const Segment& s : segments) {
        ...
        time_analytic.Start();
        EXPECT_TRUE(opt.computeSegmentMaximumMagnitudeCandidates(kDerivative, s, 0, s.getTime(), &res));
        time_analytic.Stop();

        std::vector<double> res_sampling;
        time_sampling.Start();
        const double dt = 0.01;
        opt.computeSegmentMaximumMagnitudeCandidatesBySampling<kDerivative>(s, 0, s.getTime(), dt,
                                                                            &res_sampling);
        time_sampling.Stop();
        ...
        if (!success) {
            ...
            std::cout << "segment idx: " << segment_idx << "/" << segments.size() << " time: " << s.getTime()
                      << std::endl
                      << "segment: " << s << std::endl;
    ...
    double v_max_ref = getMaximumMagnitude(trajectory, derivative_order::VELOCITY);
    double a_max_ref = getMaximumMagnitude(trajectory, derivative_order::ACCELERATION);

    timing::Timer time_analytic_v("time_extrema_analytic_v" + getSuffix());
    Extremum v_max_opt = opt.computeMaximumOfMagnitude<derivative_order::VELOCITY>(nullptr);
    time_analytic_v.Stop();
    timing::Timer time_analytic_a("time_extrema_analytic_a" + getSuffix());
    Extremum a_max_opt = opt.computeMaximumOfMagnitude<derivative_order::ACCELERATION>(nullptr);
    time_analytic_a.Stop();

    Extremum v_min_traj, v_max_traj, a_min_traj, a_max_traj;

    trajectory.computeMinMaxMagnitude(derivative_order::VELOCITY, dimensions, &v_min_traj, &v_max_traj);
    trajectory.computeMinMaxMagnitude(derivative_order::ACCELERATION, dimensions, &a_min_traj, &a_max_traj);
...
TEST_P(PolynomialOptimizationTests, UnconstrainedNonlinear) {
    ...
    std::cout << "nlopt1 stopped for reason: " << nlopt::returnValueToString(ret) << std::endl;

    timing::Timer timer_setup2("setup_nonlinear_time_and_derivatives" + getSuffix());
    ...
    timer_setup2.Stop();
    timing::Timer timer_solve2("solve_nonlinear_time_and_derivatives" + getSuffix());
    ret = opt2.optimize();
    timer_solve2.Stop();
    ...
    std::cout << "nlopt2 stopped for reason: " << nlopt::returnValueToString(ret) << std::endl;
    ...
    double a_max_1 = getMaximumMagnitude(trajectory1, derivative_order::ACCELERATION);
    std::cout << "v_max_1: " << v_max_1 << " a_max_1: " << a_max_1 << std::endl;
    EXPECT_LE(v_max_1, v_max * 1.5);
    EXPECT_LE(a_max_1, a_max * 1.5);

    EXPECT_TRUE(
        checkCost(opt.getPolynomialOptimizationRef().computeCost(), trajectory1, max_derivative, 0.1));

    double v_max_2 = getMaximumMagnitude(trajectory2, derivative_order::VELOCITY);
    double a_max_2 = getMaximumMagnitude(trajectory2, derivative_order::ACCELERATION);
    ...
    EXPECT_TRUE(
        checkCost(opt2.getPolynomialOptimizationRef().computeCost(), trajectory2, max_derivative, 0.1));
}
...
TEST_P(PolynomialOptimizationTests, ConstraintPacking) {
    ...
    for (size_t i = 0; i < kNumSetups; i++) {
        ...
        vertices = createRandomVertices(kMaxDerivative, kNumSegments, min_pos, max_pos, kSeed + i);

        const double kApproxVMax = 3.0;
        const double kApproxAMax = 5.0;
        std::vector<double> segment_times = estimateSegmentTimes(vertices, kApproxVMax, kApproxAMax);
        ...
        for (int i = 0; i < D; ++i) {
            Eigen::VectorXd d_all_ordered(fixed_constraints[i].size() + free_constraints[i].size());
...
TEST_P(PolynomialOptimizationTests, TimeAllocation) {
    ...
    segment_times_ramp = estimateSegmentTimesVelocityRamp(vertices_, v_max, a_max);
    ...
    opt_nfabian.setupFromVertices(vertices_, segment_times_nfabian, max_derivative);
    ...
    getMaxVelocityAndAccelerationAnalytical(trajectory_ramp, &v_max_ramp, &a_max_ramp);
    getMaxVelocityAndAccelerationAnalytical(trajectory_nfabian, &v_max_nfabian, &a_max_nfabian);
...
TEST_P(PolynomialOptimizationTests, TimeScaling) {
    ...
    segment_times = estimateSegmentTimesVelocityRamp(vertices_, v_max, a_max, time_factor);
    ...
    std::cout << "Starting v max: " << v_max_traj << " (" << v_max_num << ") a max: " << a_max_traj << " ("
              << a_max_num << ")" << std::endl;
    ...
    std::cout << "Scaled v max: " << v_max_traj << " a max: " << a_max_traj << std::endl;
    ...
    std::cout << "Mellinger v max: " << v_max_traj << " a max: " << a_max_traj << std::endl;
...
TEST_P(PolynomialOptimizationTests, TimeScalingInTrajectory) {
    ...
    std::vector<double> segment_times = estimateSegmentTimesVelocityRamp(vertices_, v_max * 2.0, a_max * 2.0);
    EXPECT_EQ(segment_times.size(), params_.num_segments);
    ...
    getMaxVelocityAndAccelerationAnalytical(trajectory, &v_max_traj, &a_max_traj);
    getMaxVelocityAndAccelerationNumerical(trajectory, &v_max_num, &a_max_num);
    std::cout << "Scaled v max: " << v_max_traj << " (" << v_max_num << ") a max: " << a_max_traj << " ("
              << a_max_num << ")" << std::endl;
...
TEST_P(PolynomialOptimizationTests, AMatrixInversion) {
    ...
    for (double t = 1; t <= max_time; t += 1) {
        ...
        Ai_eigen = A.inverse();
        EXPECT_TRUE(EIGEN_MATRIX_NEAR(Ai, Ai_eigen, 1.0e-10)) << "time was " << t << std::endl;
    }
}
...
TEST_P(PolynomialOptimizationTests, TwoVerticesSetup) {
    ...
    matlab_coeffs << -0.000000000000004, 0.000000000000004, -0.000000000000006, 0.000000000000003,
        -0.000000000000001, 0.201600000000015, -0.134400000000012, 0.034560000000004, -0.004032000000000,
        0.000179200000000;
...
INSTANTIATE_TEST_CASE_P(OneDimension, PolynomialOptimizationTests,
                        ::testing::Values(segment_1_dim_1, segment_10_dim_1, segment_50_dim_1));

INSTANTIATE_TEST_CASE_P(ThreeDimensions, PolynomialOptimizationTests,
                        ::testing::Values(segment_1_dim_3, segment_10_dim_3, segment_50_dim_3));

INSTANTIATE_TEST_CASE_P(Derivatives, PolynomialOptimizationTests,
                        ::testing::Values(deriv_accel_1, deriv_accel_3_1, deriv_accel, deriv_jerk));
