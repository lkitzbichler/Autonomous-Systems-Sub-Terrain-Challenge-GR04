...
struct EigenAttitudeThrust {
    EigenAttitudeThrust() : attitude(Eigen::Quaterniond::Identity()), thrust(Eigen::Vector3d::Zero()) {}
...
struct EigenActuators {
    ...
    EigenActuators(const Eigen::VectorXd& _angular_velocities) { angular_velocities = _angular_velocities; }
...
struct EigenRateThrust {
    EigenRateThrust() : angular_rates(Eigen::Vector3d::Zero()), thrust(Eigen::Vector3d::Zero()) {}
...
struct EigenTorqueThrust {
    EigenTorqueThrust() : torque(Eigen::Vector3d::Zero()), thrust(Eigen::Vector3d::Zero()) {}
...
struct EigenRollPitchYawrateThrust {
    EigenRollPitchYawrateThrust() : roll(0.0), pitch(0.0), yaw_rate(0.0), thrust(Eigen::Vector3d::Zero()) {}
...
class EigenMavState {
   public:
    ...
    EigenMavState(const Eigen::Vector3d& position_W, const Eigen::Vector3d& velocity_W,
                  const Eigen::Vector3d& acceleration_B, const Eigen::Quaterniond& orientation_W_B,
    ...
    std::string toString() const {
        ...
        ss << "position:              " << position_W.transpose() << std::endl
           ...
           << "angular_acceleration_body: " << angular_acceleration_B.transpose() << std::endl;
...
struct EigenTrajectoryPoint {
    ...
    EigenTrajectoryPoint(int64_t _time_from_start_ns, const Eigen::Vector3d& _position,
                         const Eigen::Vector3d& _velocity, const Eigen::Vector3d& _acceleration,
                         const Eigen::Vector3d& _jerk, const Eigen::Vector3d& _snap,
                         const Eigen::Quaterniond& _orientation, const Eigen::Vector3d& _angular_velocity,
    ...
    EigenTrajectoryPoint(int64_t _time_from_start_ns, const Eigen::Vector3d& _position,
                         ...
                         const Eigen::Quaterniond& _orientation, const Eigen::Vector3d& _angular_velocity,
                         const MavActuation& _degrees_of_freedom = MavActuation::DOF4)
        : EigenTrajectoryPoint(_time_from_start_ns, _position, _velocity, _acceleration, _jerk, _snap,
                               _orientation, _angular_velocity, Eigen::Vector3d::Zero(),
                               _degrees_of_freedom) {}
    ...
    inline void setFromYaw(double yaw) { orientation_W_B = quaternionFromYaw(yaw); }
...
struct EigenOdometry {
    ...
    inline Eigen::Vector3d getVelocityWorld() const { return orientation_W_B * velocity_B; }
...
#define MAV_MSGS_MAKE_ALIGNED_CONTAINERS(EIGEN_TYPE)                                                        \
    typedef std::vector<EIGEN_TYPE, Eigen::aligned_allocator<EIGEN_TYPE>> MAV_MSGS_CONCATENATE2(EIGEN_TYPE, \
                                                                                                Vector);    \
    typedef std::deque<EIGEN_TYPE, Eigen::aligned_allocator<EIGEN_TYPE>> MAV_MSGS_CONCATENATE2(EIGEN_TYPE,  \
                                                                                               Deque);
