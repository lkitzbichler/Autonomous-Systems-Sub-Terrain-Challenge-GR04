...
inline double MagnitudeOfGravity(const double height, const double latitude_radians) {
    ...
    double sin_squared_twice_latitude = std::sin(2 * latitude_radians) * std::sin(2 * latitude_radians);
    return kGravity_0 * ((1 + kGravity_a * sin_squared_latitude - kGravity_b * sin_squared_twice_latitude) -
                         kGravity_c * height);
}
...
// Peres
inline Eigen::Vector3d omegaFromRotationVector(const Eigen::Vector3d& rot_vec,
...
// based on formula derived in "Finite rotations and angular velocity" by Asher
// Peres
inline Eigen::Vector3d omegaDotFromRotationVector(const Eigen::Vector3d& rot_vec,
                                                  const Eigen::Vector3d& rot_vec_vel,
    ...
    W_vel = phi_skew * (phi * std::sin(phi) - 2.0f + 2.0f * std::cos(phi)) * phi_dot * phi_3_inv +
            phi_skew * phi_skew * (-2.0f * phi - phi * std::cos(phi) + 3.0f * std::sin(phi)) * phi_dot *
                phi_4_inv +
            phi_dot_skew * phi_skew * (phi - std::sin(phi)) * phi_3_inv;

    W_acc = Eigen::MatrixXd::Identity(3, 3) + phi_skew * (1.0f - std::cos(phi)) * phi_2_inv +
            phi_skew * phi_skew * (phi - std::sin(phi)) * phi_3_inv;
...
inline void getSquaredRotorSpeedsFromAllocationAndState(const Eigen::MatrixXd& allocation_inv,
                                                        const Eigen::Vector3d& inertia, double mass,
    ...
    const Eigen::Vector3d torque = inertia.asDiagonal() * angular_acceleration_B +
...
inline int64_t secondsToNanoseconds(double seconds) {
    int64_t nanoseconds = static_cast<int64_t>(seconds * kNumNanosecondsPerSecond);
    return nanoseconds;
}
