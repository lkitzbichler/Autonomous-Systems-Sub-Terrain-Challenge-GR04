...
class QuadrotorLCMInterface {
   public:
    ...
    bool Good() const { return lcm.good(); }

    void Spin() { lcm.handle(); }
    ...
    void RegisterIMUCallback(
        std::function<void(Eigen::Vector3f, Eigen::Vector3f, Eigen::Vector3f)> callback) {
        imu_callback = callback;
    }

    void RegisterStateCallback(
        std::function<void(Eigen::Vector3f, Eigen::Vector4f, Eigen::Vector3f, Eigen::Vector3f)> callback) {
        state_callback = callback;
    }

   private:
    void OnOutput(const lcm::ReceiveBuffer* rbuf, const std::string& chan,
                  const drake::lcmt_quadrotor_output_t* msg) {
        if (imu_callback) {
            // converted to PX4 frame
            imu_callback(
                Eigen::Vector3f(msg->accelerometer[0], -msg->accelerometer[1], -msg->accelerometer[2]),
        ...
        if (state_callback) {
            Eigen::Vector4f orientation = Eigen::Vector4f(msg->orientation[0], msg->orientation[1],
                                                          msg->orientation[2], msg->orientation[3]);
            orientation.normalize();
            state_callback(Eigen::Vector3f(msg->position[0], msg->position[1], msg->position[2]), orientation,
