...
class UDPPoseStreamerNode final : public rclcpp::Node {
   public:
    UDPPoseStreamerNode() : rclcpp::Node("w_to_unity"), dgram_client_(LIBSOCKET_IPv4) {
        ...
        RCLCPP_INFO(get_logger(), "UDP streamer to %s:%s at 1 kHz (offsets x=%.3f, y=%.3f)",
        ...
        sub_ = this->create_subscription<Actuators>(
            "rotor_speed_cmds", rclcpp::QoS(1), [this](const Actuators::SharedPtr msg) {
                ...
                RCLCPP_DEBUG(this->get_logger(), "Received w: %.3f %.3f %.3f %.3f", last_w_[0], last_w_[1],
                             last_w_[2], last_w_[3]);
            });

        // Timer at 1 kHz: send latest 4 floats if valid
        timer_ = this->create_wall_timer(std::chrono::milliseconds(1), [this]() {
            // Copy to local buffer to avoid partial reads if callback updates in
            // between
            ...
            try {
                ...
                RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "UDP send failed: %s",
                                     e.mesg.c_str());
            }
        });
    }
