...
class TrueStateParser : public UnityStreamParser {
   public:
    ...
    virtual bool ParseMessage(const UnityHeader& header, TCPStreamReader& stream_reader,
        ...
        if (pose_publishers_.find(header.name) == pose_publishers_.end()) {
            pose_publishers_.insert(std::make_pair(
                header.name,
                node_->create_publisher<geometry_msgs::msg::PoseStamped>(header.name + "/pose", 10)));
            twist_publishers_.insert(std::make_pair(
                header.name,
                node_->create_publisher<geometry_msgs::msg::TwistStamped>(header.name + "/twist", 10)));
        }
   ...
   private:
    std::unordered_map<std::string, rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr>
        pose_publishers_;
    std::unordered_map<std::string, rclcpp::Publisher<geometry_msgs::msg::TwistStamped>::SharedPtr>
        twist_publishers_;
