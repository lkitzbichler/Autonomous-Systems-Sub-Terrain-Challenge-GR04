...
class RGBCameraParser : public UnityStreamParser {
   public:
    RGBCameraParser() { node_ = rclcpp::Node::make_shared("rgb_camera_parser"); }

    virtual bool ParseMessage(const UnityHeader& header, TCPStreamReader& stream_reader,
   ...
   protected:
    virtual void PublishImage(const UnityHeader& header, const ImageData& img, double time_offset) {
        if (image_publishers_.find(header.name) == image_publishers_.end()) {
            image_publishers_.insert(std::make_pair(
                header.name,
                node_->create_publisher<sensor_msgs::msg::Image>(header.name + "/image_raw", 10)));
            info_publishers_.insert(std::make_pair(
                header.name,
                node_->create_publisher<sensor_msgs::msg::CameraInfo>(header.name + "/camera_info", 10)));
        }
    ...
    virtual void ImageToRos(const ImageData& img, const std::string& frame,
   ...
   private:
    ...
    std::unordered_map<std::string, rclcpp::Publisher<sensor_msgs::msg::CameraInfo>::SharedPtr>
        info_publishers_;
};
