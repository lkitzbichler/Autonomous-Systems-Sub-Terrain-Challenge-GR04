...
class UnityCommandStream {
   public:
    UnityCommandStream(std::string host, std::string port) {
        ...
        socket_ptr_ = std::unique_ptr<libsocket::inet_stream>(
            new libsocket::inet_stream(host, port, LIBSOCKET_IPv4, 0));
    }

    void SendCommand(std::string command) { (*socket_ptr_) << command; }

   private:
    void CommandCallback(const std_msgs::msg::String::SharedPtr command) { SendCommand(command->data); }
