...
void TCPImageServer::WaitConnect() { m_stream_reader->WaitConnect(); }
...
bool TCPImageServer::Good() { return m_stream_reader->Good(); }
...
ImageData TCPImageServer::FlipImage(ImageData const& img) {
    ...
    for (int row = 0; row < img.height; row++) {
        memcpy(flipped_image.data.get() + row * stride, img.data.get() + (img.height - row - 1) * stride,
               stride);
    }
