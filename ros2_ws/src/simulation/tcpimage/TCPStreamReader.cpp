...
uint64_t TCPStreamReader::ReadUInt64() {
    ...
    if (bytes_received != sizeof(uint64_t)) {
        throw socket_exception("TCPStreamReader", 54,
                               "Failed to read unsigned 64-bit int from server stream");
    }
...
shared_ptr<uint8_t> TCPStreamReader::ReadBytes(const size_t num_bytes) {
    ...
    if (m_server_stream != nullptr) {
        ...
        while (data_remaining > 0) {
            bytes_received =
                m_server_stream->rcv(bytes.get() + current_offset, min(data_remaining, CHUNK_SIZE), 0);
...
bool TCPStreamReader::Good() { return m_good; }
