...
BasicPlanner::BasicPlanner(const rclcpp::Node::SharedPtr& node)
    ...
    pub_trajectory_ =
        node_->create_publisher<mav_planning_msgs::msg::PolynomialTrajectory4D>("trajectory", 10);
...
bool BasicPlanner::planTrajectory(const Eigen::VectorXd& start_pos, const Eigen::VectorXd& start_vel,
...
void BasicPlanner::drawMavTrajectory(const mav_trajectory_generation::Trajectory& trajectory,
    ...
    bool success = mav_trajectory_generation::sampleWholeTrajectory(trajectory, 0.1, &trajectory_points);
    if (!success) {
